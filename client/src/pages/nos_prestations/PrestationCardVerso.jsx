...
export default function PrestationCardRecto({
	prestations,
	prestation,
	theme,
	scrollToNext,
	...
	return (
		<div
			className={`absoluteflex flex-col justify-between w-full h-full transition-transform duration-700 transform-style-preserve-3d backface-hidden ${
				flippedIndexes.includes(index) ? "rotate-y-0" : "rotate-y-180"
			}`}
		>
			<div className='lg:mt-46 lg:mb-18'>
				<RotatingCircle
					className='max-w-92 md:max-w-92 lg:max-w-none lg:h-124 -translate-x-24 lg:-translate-x-58 -translate-y-36 lg:-translate-y-78'
					doAnimate={false}
				/>
				<div className='flex flex-col lg:flex-row items-center space-y-8 lg:space-y-0 lg:space-x-16 justify-between'>
					...
					<RotatingCircle
						className='max-w-92 md:max-w-92 lg:max-w-none lg:h-136 -translate-x-38 translate-y-64'
						doAnimate={false}
					/>
					...
					{imageSrc && (
						<img
							...
							className={`absolute right-0 w-156 h-auto object-cover opacity-50 ${
								prestation.image?.parameters?.className || ""
							}`}
						/>
					)}
				</div>
